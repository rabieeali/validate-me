...
function isFarsi(input: string, ignoredChars: string[] = []): boolean {
    const ignoredRegex = new RegExp(`[${ignoredChars.join("")}]`, "g");
    const sanitizedInput = input.replace(ignoredRegex, "");
